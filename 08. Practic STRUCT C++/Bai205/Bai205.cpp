...
PHANSO AmDauTien(PHANSO[], int);
PHANSO AmLonNhat(PHANSO[], int);
...
PHANSO AmDauTien(PHANSO a[], int n)
{
	for (int i = 0; i < n; i++)
	{
		if (a[i].Tu * a[i].Mau < 0)
...
PHANSO AmLonNhat(PHANSO a[], int n)
{
	PHANSO lc = AmDauTien(a, n);
	for (int i = 0; i < n; i++)
	{
		if (a[i].Tu * a[i].Mau < 0)
		{
