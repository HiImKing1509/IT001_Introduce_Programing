...
float GanO(DUONGTRON);
DUONGTRON GanONhat(DUONGTRON[], int);
...
float GanO(DUONGTRON c)
{
	return abs(KhoangCachIO(c) - c.R);
}
DUONGTRON GanONhat(DUONGTRON a[], int n)
{
	DUONGTRON c1 = a[0];
	for (int i = 1; i < n; i++)
		if (GanO(a[i]) < GanO(c1))
